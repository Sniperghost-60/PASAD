...
const COLS = [
    { key: 'commune',             label: 'Commune',                                    w: 100 },
    { key: 'arrondissement',      label: 'Arrondissement',                             w: 120 },
    { key: 'village',             label: 'Village',                                    w: 100 },
    { key: 'nom_producteur',      label: 'Nom et prénoms du producteur',               w: 180 },
    { key: 'categorie_pesticide', label: 'Catégories de pesticides chimiques',         w: 160 },
    { key: 'qte_n2',              label: 'Qté utilisée (L) en année n-2',              w: 130 },
    { key: 'qte_n1',              label: 'Qté utilisée (L) en année n-1',              w: 130 },
    { key: 'qte_n',               label: 'Qté utilisée (L) en année n',               w: 130 },
    { key: 'observations',        label: 'Observation',                               w: 130 },
];

const NUM_KEYS = ['qte_n2', 'qte_n1', 'qte_n'];

const emptyLigne = () => ({
    commune: '', arrondissement: '', village: '', nom_producteur: '',
    categorie_pesticide: '', qte_n2: '', qte_n1: '', qte_n: '', observations: '',
});

const emptyDonnees = () => ({
    lignes: [
        { ...emptyLigne(), categorie_pesticide: 'Herbicides chimiques' },
        { ...emptyLigne(), categorie_pesticide: 'Insecticides chimiques' },
        emptyLigne(),
    ],
});

const num = v => parseFloat(String(v).replace(/\s/g, '').replace(',', '.')) || 0;
const fmtNum = v => { const n = num(v); return n === 0 && v === '' ? '' : n.toLocaleString('fr-FR'); };
...
const PRINT_STYLES = [
    ...
    '.total-row td{background:#fee2e2;font-weight:700;color:#7f1d1d}',
    '.title-box{text-align:center;margin-bottom:6px}',
    '.main-title{font-size:13px;font-weight:700}',
...
export default function CaiEvolutionProduitsChimiques() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        try {
            let url = `/api/cai/evolution-produits-chimiques?date_session=${dateSession}`;
    ...
    const handlePrint = () => {
        ...
        w.document.write('<html><head><meta charset="utf-8"><title>Produits chimiques</title><style>' + PRINT_STYLES + '</style></head><body>');
        w.document.write('<div class="title-box">');
        w.document.write('<div class="main-title">Modèle de tableau d\'évolution des quantités de produits chimiques de synthèse utilisés</div>');
        w.document.write('<div class="sub-title">Phase 5 — Évaluation | Étape 18 | Session : ' + dateSession + '</div>');
    ...
    const renderTable = (forPrint = false) => (
        <div style={{ overflowX: 'auto' }}>
            <table style={{ borderCollapse: 'collapse', minWidth: '1200px', width: '100%', fontSize: '13px' }}>
                <thead>
                    <tr>
                        {COLS.map(c => (
                            <th key={c.key} style={{
                        ...
                        {!forPrint && editMode && (
                            <th style={{ background: AMBER_DARK, color: '#fff', padding: '7px 8px', border: '1px solid #92400e', width: 50 }}>#</th>
                        )}
                    </tr>
                </thead>
                <tbody>
                                    ...
                                    {forPrint || !editMode ? (
                                        ...
                                        <input
                                            ...
                                            style={{
                                                ...
                                                color: NUM_KEYS.includes(c.key) ? RED : '#111827',
                                                padding: '2px 4px',
                                            }}
                                        />
                                    )}
                                ...
                                <td style={{ border: '1px solid #d1d5db', textAlign: 'center', padding: '2px' }}>
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: donnees.lignes.length <= 1 ? 'not-allowed' : 'pointer',
                                            color: '#ef4444', fontSize: '16px',
                                            opacity: donnees.lignes.length <= 1 ? 0.3 : 1,
                                        }}
                                        title="Supprimer"
                                    >✕</button>
                                </td>
                    ...
                    {/* Ligne Total */}
                    <tr className="total-row" style={{ background: RED_BG }}>
                        <td colSpan={4} style={{ border: '1px solid #d1d5db', padding: '5px 8px', fontWeight: 700, color: '#7f1d1d', fontSize: '13px' }}>
                            Total
                        </td>
                        <td style={{ border: '1px solid #d1d5db', padding: '5px 8px' }} />
                        {NUM_KEYS.map(k => (
                            <td key={k} style={{ border: '1px solid #d1d5db', padding: '5px 8px', fontWeight: 700, color: '#7f1d1d', textAlign: 'center' }}>
                                {totaux[k] > 0 ? totaux[k].toLocaleString('fr-FR') : ''}
                            </td>
                        ))}
                        <td style={{ border: '1px solid #d1d5db' }} />
                        {!forPrint && editMode && <td style={{ border: '1px solid #d1d5db' }} />}
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                <main style={{ flex: 1, padding: '24px', overflowX: 'auto' }}>
                    ...
                    <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,.08)', padding: '20px 24px', marginBottom: 20, borderLeft: `5px solid ${AMBER_DARK}` }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                            <div>
                                <div style={{ fontSize: 17, fontWeight: 700, color: AMBER_DARK }}>
                                    Évolution des quantités de produits chimiques de synthèse utilisés
                                </div>
                                <div style={{ fontSize: 13, color: '#6b7280', marginTop: 2 }}>
                                    Phase 5 — Évaluation et actualisation du diagnostic | Étape 18
                                </div>
                            </div>
                        ...
                        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                            <div style={{ background: '#fff', borderRadius: 12, maxWidth: '95vw', maxHeight: '90vh', overflow: 'auto', padding: 24, position: 'relative', minWidth: 700 }}>
                                ...
                                <div style={{ textAlign: 'center', marginBottom: 12 }}>
                                    <div style={{ fontWeight: 700, fontSize: 15, color: AMBER_DARK }}>Évolution des quantités de produits chimiques de synthèse utilisés</div>
                                    <div style={{ fontSize: 12, color: '#6b7280' }}>Phase 5 — Évaluation | Étape 18 | Session : {dateSession}</div>
                                </div>
                                <div ref={printRef}>
                                    {renderTable(true)}
                                </div>
